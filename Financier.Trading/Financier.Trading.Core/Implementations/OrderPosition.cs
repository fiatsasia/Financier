...
namespace Financier.Trading
{
    public class OrderPosition
    {
        ...
        internal OrderPosition(OrderPositions positions, ActivePosition pos)
        {
            Positions = positions;
        ...
        internal OrderPosition(OrderPositions positions, ActivePosition pos, OrderExecution exec)
        {
            Positions = positions;
    ...
    public class OrderPositions
    {
        ...
        public IEnumerable<OrderPosition> GetOpenPositions()
        {
            return _q.ToList().Select(e => new OrderPosition(this, e));
        }

        public IEnumerable<OrderPosition> Update(OrderExecution exec)
        {
            if (_q.Count == 0 || Math.Sign(_q.Peek().OpenSize) == Math.Sign(exec.Size))
            {
                var pos = new ActivePosition(exec);
                _q.Enqueue(pos);
                return new OrderPosition[] { new OrderPosition(this, pos) };
            }
            ...
            var result = closedPos.Select(e => new OrderPosition(this, e, exec)).ToList();

            if (closingSize > 0m)
            {
                var pos = new ActivePosition(exec);
                _q.Enqueue(pos);
                result.Add(new OrderPosition(this, pos));
            }
